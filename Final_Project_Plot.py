...
while True:
    ...
    for i in range(len(scores)):
        if scores[i] > args.threshold:
            ...
            # Track entry and exit times for objects
            if object_name not in object_entry_exit_times:
                object_entry_exit_times[object_name] = {'entry': time.time() - start_time, 'exit': None}
            object_entry_exit_times[object_name]['exit'] = time.time() - start_time
    ...
    frame_rate_calc = 1.0 / inference_time
...
# Inference Times
plt.subplot(4, 1, 4)
plt.plot(timestamps, inference_times, label='Inference Time', color='purple')
for obj, times in object_entry_exit_times.items():
    plt.scatter(times['entry'], 0.5, label=f"{obj} Entry", color='orange')
    plt.scatter(times['exit'], 0.5, label=f"{obj} Exit", color='cyan')
plt.title("Inference Times and Entry/Exit Times")
plt.xlabel("Time (s)")
plt.ylabel("Time (s)")
plt.legend()
